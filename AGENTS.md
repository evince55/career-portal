...
| `service-worker.js` | PWA cache **`career-portal-v15`**. Network-first for navigations, cache-first (ignoreSearch) for assets. Precache list is validated against disk by `tests/site-integrity.mjs`. |
...
- **Service worker cache name must be bumped** (`career-portal-v16`, …) whenever cached assets
