...
| `service-worker.js` | PWA cache **`career-portal-v22`**. Network-first for navigations, cache-first (ignoreSearch) for assets. Precache list is validated against disk by `tests/site-integrity.mjs`. |
...
- **Service worker cache name must be bumped** (`career-portal-v23`, …) whenever cached assets
